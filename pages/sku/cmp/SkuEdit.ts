...
export default class SkuEdit extends Vue {
  ...
  created() {
    ...
    if (this.id && this.id.length > 0) {
      ...
      SkuApi.get(this.id).then((resp) => {
        ...
        this.getPageList()
        resp.data.skuFormulaList && resp.data.skuFormulaList.forEach((item, index) => {
          this.selMulArray[index] = item.formulaSku!.id
          this.selMulIndexArray[index] = item.qty
        })
      }).catch(error => {
  ...
  @Watch('sku.type')
  onWatchType(value: string) {
    if (value && value !== 'NORMAL') {
      let mulItem = new SkuFormula()
      if (this.sku.skuFormulaList && this.sku.skuFormulaList[this.sku.skuFormulaList.length - 1].formulaSku!.munit! && this.sku.skuFormulaList[this.sku.skuFormulaList.length - 1].qty) {
        this.sku.skuFormulaList.push(mulItem)
      } else {
        this.sku.skuFormulaList = []
        this.sku.skuFormulaList.push(mulItem)
      }
    }
  }
  @Watch('isMulFlag')
  onMulFlagWatch(value: boolean) {
    if (!value) {
      this.sku.skuFormulaList = []
    } else {
      this.sku.skuFormulaList = []
  ...
  getUnit() {
    ...
    query.filters =
      [ { property: 'type:=', value: 'FORMULA' } ]
    SkuApi.query(query).then((resp) => {
      // todo
      if (resp.success && resp.data) {
        this.mUnitArray = resp.data
      }
    }).catch((error) => {
      this.$message.error(error.message)
    })
  ...
  onDelMulSku(index: number) {
    this.sku.skuFormulaList.splice(index, 1)
    this.selMulArray.splice(index, 1)
    this.selMulIndexArray.splice(index, 1)
  }
  ...
  onTranslate() {
    this.selMulArray.forEach((item, index) => {
      let itemObj = new ThinSku()
      let itemSkuMul = new SkuFormula()
      let subObj: any = ''
      subObj = this.mUnitArray.filter(sub => {
        return sub.id === item
      })
      if (subObj.length > 0) {
        itemObj.code = subObj[0].code
        itemObj.id = subObj[0].id
        itemObj.name = subObj[0].name
        itemObj.munit = subObj[0].munit
      }
      itemSkuMul.formulaSku = itemObj
      itemSkuMul.qty = this.selMulIndexArray[index]
      this.sku.skuFormulaList[index] = (itemSkuMul as any)
    })
  }
  onSave() {
